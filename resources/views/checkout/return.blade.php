<x-layouts.app :title="'Confirmación '.$order->reference.' — XpertSystems'" description="Estado del anticipo de tu proyecto con XpertSystems.">
    <div class="checkout-page">
        ...
        <main class="return-card">
            @if($order->status === \App\Enums\OrderStatus::DepositPaid)
                <span class="return-status">Anticipo confirmado</span>
                <h1>Gracias. Tu proyecto ya tiene un primer paso firme.</h1>
                <p>Recibimos el anticipo de <strong>${{ number_format((float)$order->deposit_amount, 0) }} MXN</strong> para {{ $order->package->name }}. Te contactaremos con los siguientes pasos.</p>
            ...
            @else
                ...
                <p>La plataforma de pago todavía no confirma el anticipo. Conserva esta referencia: <strong>{{ $order->reference }}</strong>. Actualizaremos el estado cuando recibamos la confirmación segura.</p>
            @endif
