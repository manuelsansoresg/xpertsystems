<x-layouts.app
    ...
    description="Completa tus datos y paga el anticipo para comenzar tu proyecto con XpertSystems."
>
                ...
                <div class="checkout__grid">
                    <form method="POST" action="{{ route('checkout.store', $package) }}" class="checkout-form" data-checkout-form>
                        ...
                        <div class="checkout-submit">
                            <button type="submit" class="button button--gold button--full" data-analytics="begin_checkout" data-package="{{ $package->slug }}">Pagar anticipo <span>→</span></button>
                    ...
                    <aside class="order-summary" aria-label="Resumen de la orden">
                        ...
                        <h2>{{ $package->name }}</h2>
                        <div class="summary-row"><span>Precio total</span><strong>${{ number_format((float)$package->price, 0) }} MXN</strong></div>
                        <div class="summary-row summary-row--deposit"><span>Anticipo hoy</span><strong>${{ number_format($package->deposit_amount, 0) }} MXN</strong></div>
                        <div class="summary-row"><span>Pendiente al finalizar</span><strong>${{ number_format((float)$package->price - $package->deposit_amount, 0) }} MXN</strong></div>
