<x-layouts.admin title="Editar paquete">
    ...
    <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="admin-form" x-data="packageForm({{ Js::from($package->featureItems->map(fn ($f) => ['title' => $f->title, 'visible_summary' => $f->visible_summary])->values()) }})">
        @csrf
        @method('PUT')
