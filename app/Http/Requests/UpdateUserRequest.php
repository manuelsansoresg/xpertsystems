...
final class UpdateUserRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($this->input('role') === 'seller') {
            ...
            $existingProfile = $userId ? \App\Models\SellerProfile::where('user_id', $userId)->first() : null;
            ...
            $rules['referral_code'] = [
                ...
                'max:20',
                'regex:/^[A-Z0-9]+$/',
                Rule::unique('seller_profiles', 'referral_code')->ignore($existingProfile?->id),
            ];
            $rules['commission_type'] = ['required', Rule::in(['percentage', 'fixed'])];
            $rules['commission_value'] = ['required', 'numeric', 'min:0', 'max:10000'];
